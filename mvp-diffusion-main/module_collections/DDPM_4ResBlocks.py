...
class DoubleConv(nn.Module):
    ...
    def forward(self, x):
        if self.residual:
            scale = torch.tensor(1)/torch.sqrt(torch.tensor(2)) # rescale residual link
            return F.gelu(x * scale + self.double_conv(x))
        else:
...
class DDPM_4ResBlocks(nn.Module):
...
if __name__ == '__main__':
    net = DDPM_4ResBlocks(device="cpu")
