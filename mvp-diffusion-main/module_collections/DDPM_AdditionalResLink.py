...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256):
        ...
        self.conv = nn.Sequential(
            DoubleConv(in_channels, in_channels, residual=True),
            DoubleConv(in_channels, out_channels, in_channels // 2, residual=True),
        )
...
class DDPM_AdditionalResLink(nn.Module):
    def __init__(self, c_in=3, c_out=3, time_dim=256, device="cuda"):
        super().__init__()
        self.device = device
        self.time_dim = time_dim
        self.inc = DoubleConv(c_in, 64, residual=True)
        ...
        self.bot1 = DoubleConv(256, 512, residual=True)
        self.bot2 = DoubleConv(512, 512, residual=True)
        self.bot3 = DoubleConv(512, 256, residual=True)
...
if __name__ == '__main__':
    net = DDPM_AdditionalResLink(device="cpu")
