...
class Down_BigGAN_Residual(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256):
        super().__init__()
        self.maxpool_conv = nn.Sequential(
            nn.ReLU(),
    ...
    def forward(self, x, t):
        h1 = self.maxpool_conv(x)
        h2 = self.residual_conv(x)
        emb = self.emb_layer(t)[:, :, None, None].repeat(1, 1, x.shape[-2], x.shape[-1])
        return h1 + h2 + emb

class Up_BigGAN_Residual(nn.Module):
    def __init__(self, in_channels, out_channels, emb_dim=256):
        super().__init__()
        ...
        self.conv_3 = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=1, bias=False)
    ...
    def forward(self, x, skip_x, t):
        h1 = self.batch_norm_1(x)
        h1 = self.activation(h1)
        h1 = self.up(h1)
        h1 = torch.cat([skip_x, h1], dim=1)
        h1 = self.conv_1(h1)
        h1 = self.batch_norm_2(h1)
        h1 = self.activation(h1)
        h1 = self.conv_2(h1)
...
class UNet_BigGAN_Residual(nn.Module):
    def __init__(self, c_in=3, c_out=3, time_dim=256, device="cuda"):
        super().__init__()
        self.device = device
        self.time_dim = time_dim
...
class Down_blur(nn.Module):
    def __init__(self, in_channels, out_channels, device, emb_dim=256):
        ...
        # Precalculated and normalised filter
        self.filter = torch.Tensor([[0.0625, 0.125, 0.0625],[0.125, 0.25, 0.125],[0.0625, 0.125, 0.0625]])
        self.filter = self.filter.repeat(in_channels, in_channels, 1, 1)
        self.filter = self.filter.to(device)

    def forward(self, x, t):
        x = F.conv2d(x, self.filter, padding='same')
        x = self.maxpool_conv(x)
        emb = self.emb_layer(t)[:, :, None, None].repeat(1, 1, x.shape[-2], x.shape[-1])
        return x + emb

class UNet_anti_aliasing(nn.Module):
    def __init__(self, c_in=3, c_out=3, time_dim=256, device="cuda"):
        super().__init__()
        self.device = device
        self.time_dim = time_dim
        self.inc = DoubleConv(c_in, 64)
        self.down1 = Down_blur(64, 128, device)
        self.sa1 = SelfAttention(128, 16)
        self.down2 = Down_blur(128, 256, device)
        self.sa2 = SelfAttention(256, 8)
        self.down3 = Down_blur(256, 256, device)
        self.sa3 = SelfAttention(256, 4)
...
if __name__ == '__main__':
    net = UNet_anti_aliasing(device="cpu")
